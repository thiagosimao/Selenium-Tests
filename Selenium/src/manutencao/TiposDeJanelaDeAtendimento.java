...
public class TiposDeJanelaDeAtendimento {
	...
	@Test
	public void testTiposDeJanelaDeAtendimento() throws Exception {
		...
		driver.findElement(By.xpath("//html/body/div[2]/div/div/div/div/div/ul/li[3]/ul/li[7]/ul/li/a")).click();

		// Adicionar Tipos de Janelas de Atendimento
		for (int second = 0;; second++) {
			if (second >= 60)
				fail("timeout");
			try {
		...
		driver.findElement(By.id("description")).sendKeys("WindowTypes_" + (int) Math.floor(Math.random() * 1000));
		String windowTypeKey = "" + (int) Math.floor(Math.random()*10000);
		driver.findElement(By.id("key")).clear();
		driver.findElement(By.id("key")).sendKeys(windowTypeKey);
		...
		// Editar Tipos de Janela de Atendimento
		for (int second = 0;; second++) {
			if (second >= 60)
				fail("timeout");
			try {
		...
		driver.findElement(By.id("searchFilter")).sendKeys(windowTypeKey);
		Thread.sleep(3000);
		...
		// Excluir Tipos de Janela de Atendimento
		for (int second = 0;; second++) {
			if (second >= 60)
				fail("timeout");
			try {
